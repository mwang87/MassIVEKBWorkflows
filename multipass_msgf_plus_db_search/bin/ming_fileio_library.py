...
def parse_table_with_headers(filename, skip_incomplete_lines=False, debug=False):
...
def parse_table_with_headers_object_list(filename):
    row_count, table_data = parse_table_with_headers(filename)
    ...
    if number_of_rows == 0:
        ...
        for key in column_dictionary:
            if len(column_dictionary[key]) != number_of_rows:
                print("Invalid number of rows for key", key)
